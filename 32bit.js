...
const morto = Module.getBaseAddress("libil2cpp.so").add(0x6828D0)
...
const option2 = {
    ...
    on() {
      ...
      const playerStats = Module.getBaseAddress("libil2cpp.so").add(0x5FEC9C)
        Interceptor.attach(playerStats,{
          onEnter(args){
        
         const enemyHealth = args[0].add(0x10).readPointer().add(0x14).readPointer()
        
         const die = Module.getBaseAddress("libil2cpp.so").add(0x5F99BC)
...
const option3 = {
    ...
    on() {
       ...
       const semrecoil = Module.getBaseAddress("libil2cpp.so").add(0x6828D0)
...
const option4 = {
    ...
    const playerStats3 = Module.getBaseAddress("libil2cpp.so").add(0x6828D0)
        Interceptor.attach(playerStats3,{
          onEnter(args){
       
          const diePlayer = args[0].add(0x18).readPointer()
          const dieYou = Module.getBaseAddress("libil2cpp.so").add(0x69219C)
...
const option5 = {
    ...
    on() {
        ...
        const libBase2 = Module.getBaseAddress("libil2cpp.so").add(0x6638E4)
          Interceptor.attach(libBase2,{
            onEnter(args){
            const iniciarExpedition = Module.getBaseAddress("libil2cpp.so").add(0x665E40)
    ...
    off() {
     ...
     const CancelStart = Module.getBaseAddress("libil2cpp.so").add(0x665E40);
        Interceptor.detachAll();
