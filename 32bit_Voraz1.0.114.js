...
class Menu {
    ...
    #createKeyInputEvent() {
        const classLoader = this.#classLoader
        const keyInputEvent = Java.registerClass({
            name: "com.example.KeyInputEvent",
            implements: [classLoader.View_OnClickListener],
            methods: {
                onClick(view) {
                    const key = prompt("Digite a chave:")
                    // Use a chave aqui como desejar
                    console.log("Chave digitada:", key)
                }
            }
        })
    this.#menuScrollLayout.addView(keyInputEvent.$new())
    }
...
Java.perform(function () {
    ...
    Java.scheduleOnMainThread(function () {
        ...
        menu.addText("Digite a chave:", 20, "#FFFFFF")
